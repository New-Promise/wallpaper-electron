...
export class WallpaperWindow {
  ...
  getScreen () {
    ...
    // 获取Y轴最小的项
    const minY = Math.min.apply(Math, screenCount.map(item => { return item.workArea.y }));
    screenCount.forEach((item: { workArea: Rectangle }) => {
      this.creationWindows(item.workArea, minY)
    })
  }

  creationWindows (params: Rectangle, minY: number) {
    const { x, y, width, height } = params;
    const YLine = Math.abs(minY) + y;
